...
<div class="max-w-2xl mx-auto my-2 md:my-8 p-6 bg-white shadow-md rounded-xl">
    <div class="flex justify-between items-center mb-4">
        ...
        <div>
            <a href="{{ route('monitoring-evaluasi.download-monev', $item->id) }}"
    ...
    <div class="mb-6">
        ...
        <div class="space-y-4">
            @foreach($evaluasiItems as $index => $evaluasiItem)
                <div class="border border-gray-200 rounded-md p-4">
                    <div class="flex items-start mb-2">
                        <span class="font-medium text-gray-800 mr-2">{{ $index + 1 }}.</span>
                        <span>{{ $evaluasiItem->pertanyaan }}</span>
                    </div>
                    <div class="mt-2">
                        <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium {{ $evaluasiItem->jawaban == 1 ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                            {{ $evaluasiItem->jawaban == 1 ? 'Ya' : 'Tidak' }}
                        </span>
                    </div>
                </div>
            @endforeach
    ...
    <div class="flex justify-between mt-8">
        ...
        <div class="space-x-2">
            <a href="{{ route('monitoring-evaluasi.edit', $item->id) }}" class="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-md transition">
                Edit
            </a>
            <form action="{{ route('monitoring-evaluasi.destroy', $item->id) }}" method="POST" class="inline-block">
                @csrf
                @method('DELETE')
