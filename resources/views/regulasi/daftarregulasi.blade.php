...
<div class="container ml-2 sm:ml-8 mt-1 sm:mt-8">
    <div class="flex items-center mb-6">
        <div class="flex border-2 border-gray-300 overflow-hidden rounded-md h-10 mr-2">
            <input type="text" id="search" placeholder="Cari Regulasi Pengawasan" autocomplete="off" class="w-96 outline-none bg-white text-gray-600 text-sm px-4 py-3" />
        </div>
        <a href="{{ route('regulasi.create') }}" class="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded-md transition">
            Tambah
        </a>
    </div>
                ...
                @forelse ($regulasi as $index => $item)
                    <tr class="hover:bg-gray-50 transition-colors" data-regulasi-row>
                        ...
                        <td class="border border-gray-300 px-4 py-3 text-center">
                            <a href="{{ route('regulasi.download', $item->id) }}" class="inline-flex px-3 py-1 text-sm font-semibold rounded bg-red-500 hover:bg-red-600 text-white transition">
                        ...
                        <td class="border border-gray-300 px-4 py-3 text-center">
                            <div class="flex justify-center space-x-2">
                                <a href="{{ route('regulasi.detail', $item->id) }}" class="bg-green-500 hover:bg-green-600 text-white px-3 py-1 rounded text-sm transition">
                                    Lihat
                                </a>
                                <a href="{{ route('regulasi.edit', $item->id) }}" class="bg-blue-500 hover:bg-blue-600 text-white px-3 py-1 rounded text-sm transition">
                                    Edit
                                </a>
                                <form action="{{ route('regulasi.delete', $item->id) }}" method="POST" class="inline-block" onsubmit="return confirm('Yakin ingin menghapus regulasi ini?')">
                                    @csrf
                                    @method('DELETE')
