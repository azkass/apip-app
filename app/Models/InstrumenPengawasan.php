...
class InstrumenPengawasan
{
    ...
    {
        return DB::selectOne('SELECT * FROM instrumen_pengawasan WHERE id = ?', [$id]);
    }
    ...
    {
        return DB::insert('INSERT INTO instrumen_pengawasan (judul, petugas_pengelola_id, isi, status, perencana_id, created_at, updated_at) VALUES (?, ?, ?, ?, ?, NOW(), NOW())', [
            $data['judul'],
    ...
    {
        return DB::update('UPDATE instrumen_pengawasan SET judul = ?, petugas_pengelola_id = ?, isi = ?, status = ?, perencana_id = ?, updated_at = NOW() WHERE id = ?', [
            $data['judul'],
            $data['petugas_pengelola_id'],
            $data['isi'],
            $data['status'],
    ...
    {
        return DB::delete('DELETE FROM instrumen_pengawasan WHERE id = ?', [$id]);
    }
}
